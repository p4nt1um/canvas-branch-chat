...
> **创建时间**: 2026-07-06
 > **更新时间**: 2026-07-11 (v9 — v0.1.0 已发布 + Skills 技术方案)
 > **状态**: ✅ v0.1.0 已发布（P0/P1/M3.5/M4 全部完成）
...
| 21 | **Claude Code Skills 集成** | 在 Canvas 节点中用 `/skill-name` 调用 Claude Code Skills。分两阶段实现（见下方专项） |
...
P2 计划新增:
├── providers/
│   ├── openai-provider.ts   ── OpenAI 兼容 Provider (从 api.ts 重构)
│   └── claude-cli-provider.ts ── Claude Code CLI Provider (阶段 A)
├── skill-scanner.ts         ── Skills 扫描 + 解析
└── skill-suggest-modal.ts   ── / 触发自动补全弹窗
```
